...
if (isset($_GET["search"])) {
    ...
    $search = " WHERE nm_marca LIKE '%" . $_GET["search"] . "%' OR id_marca = " . (int)$_GET["search"] . "";
} else {
    if (isset($_SESSION["search_value"])) {
        $search = " WHERE nm_marca LIKE '%" . $_SESSION["search_value"] . "%' OR id_marca = " . (int)$_SESSION["search_value"] . "";
    }
}
...
$sql = "SELECT id_marca, nm_marca FROM marcas $search LIMIT $registros_por_pagina OFFSET $offset";
$result = $conn->query($sql);

$total_registros_sql = "SELECT COUNT(id_marca) AS total FROM marcas $search";
...
<body>
    ...
    <div class="tabela">
        <div>
            <a href="mrc_form.php?id=0&acao=insert"><button type="submit">NOVO</button></a>
            <a href="mrc.php?show_all=1"><button type="submit">TUDO</button></a>
            <form action="mrc.php" method="GET" style="float:right;">
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="mrc_form.php?id=<?php echo $id_marca; ?>&acao=upd"><button name="editar">Editar</button></a>
                            <a href="javascript:void(0);" onclick="confirmDelete(<?php echo $id_marca; ?>)"><button name="excluir">Excluir</button></a>
                        </td>
                    </tr>
                    <?php
                }
    ...
    <script>
        function confirmDelete(id) {
            ...
            if (confirmDelete) {
                ...
                window.location.href = "mrc_del.php?id=" + id;
            } else {
