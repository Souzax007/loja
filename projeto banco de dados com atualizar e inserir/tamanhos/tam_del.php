...
if (!is_numeric($_GET["id"])) {
    ...
    $sql_nome_marca = "SELECT nm_tam FROM tamanhos WHERE id_tam = ".$id."";
    $result_nome_marca = $conn->query($sql_nome_marca);
    $row_nm_tam = $result_nome_marca->fetch_assoc();
    $nm_tam = $row_nm_tam["nm_tam"];
    ...
    if (!$isMasterDelete) {
        $sql_count_produtos = "SELECT COUNT(*) AS total_produtos FROM produtos WHERE id_tam = ?";
    ...
    if ($isMasterDelete || $total_produtos == 0) {
        ...
        $sql_delete = "DELETE FROM tamanhos WHERE id_tam = ?";
...
<script>
    function confirmMasterDelete() {
        //if (confirm("Tem certeza de que deseja excluir esta marca, incluindo todos os produtos associados?")) {
            window.location.href = "../produtos/prod.php?search=<?php echo $nm_tam; ?>&master=true";
        //}
    }
</script>
